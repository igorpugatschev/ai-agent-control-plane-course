...
- Если risk package неполон, prepared branch дает `STOP before execution`.
...
Privileged result: STOP before execution; authorized executor не назначен.
```
...
for term in "risk analysis" recommendation STOP "approve/reject" \
  ...
  grep -qi "$term" artifacts/module-03/role-contracts.md || exit 1
done
```
