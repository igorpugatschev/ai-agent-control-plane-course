...
independent review approved -> human owner for final acceptance
```
...
Implementation передает зеленый локальный тест и просит «заодно удалить stale-сценарий и сделать push». Coordinator видит, что request выходит за implementation contract. Он не направляет его reviewer-у: review не дает approval удаления или push. Он создает STOP handoff risk reviewer-у с diff, test output, файлами и вопросом: допустимы ли удаление и отправка после human approval?
...
Failure owner: risk reviewer владеет decision об approval.
Следующее действие: вернуть approve/reject/STOP с conditions и human owner.
Условие возобновления: documented approval и назначенный исполнитель.
```

Пакет не говорит «все готово». Даже после `approve` coordinator передает действие разрешенному исполнителю и сохраняет решение как evidence.
...
3. Нарисуйте routing table для пяти веток: implementation -> reviewer; finding -> implementation; test failure -> implementation/QA-SDET; irreversible request -> risk reviewer; review approval -> human owner.
...
5. В risk package укажите, что risk reviewer владеет approval, но не выполняет действие; reviewer не редактирует author diff; implementation не принимает свой результат.
...
редактирует, implementation не self-approves, risk reviewer владеет approval.
```
...
Проведите walkthrough: пакет implementation с просьбой удалить stale-сценарий должен идти `STOP -> coordinator -> risk reviewer -> documented approval or reject -> assigned executor`; reviewer не выдает approval, implementation не завершает задачу.
...
- normal review и risk approval различены;
