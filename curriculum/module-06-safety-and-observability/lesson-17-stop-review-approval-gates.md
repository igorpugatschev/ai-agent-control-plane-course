...
risk decision `approve/reject/STOP`, но по локальному contract не выполняет
delete, deploy, publish или push. Final product acceptance остается human owner.

Порядок обычно такой: reversible investigation -> author evidence -> independent
review -> complete risk package -> risk decision when needed -> human approval
for intended action -> designated executor. Approval нельзя переносить с одного
...
| Publish/deploy/delete/read secret | Привилегированно | STOP + human approval | Impact, rollback, risk package | human owner | Ничего не выполнять | Explicit approval intended action |
...
5. Запишите правило: QA/SDET сообщает test evidence, reviewer проверяет scope,
   risk reviewer дает decision при risk package, coordinator маршрутизирует,
   human owner принимает final approval. Нет self-approval или execution by
   approver.
...
- **Risk reviewer выполняет решение.** Исправление: он передает documented
  decision coordinator-у, а executor назначается отдельно.
...
4. Кто маршрутизирует package и кто принимает risk decision?
