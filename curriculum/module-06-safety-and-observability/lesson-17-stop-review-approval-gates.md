...
- Risk `STOP`/human reject заканчивает branch before execution; approve без
  отдельно named executor и evidence не завершает privileged action.
...
for term in "stop-gate" "review-gate" "human approval" "intended action" \
  ...
  "coordinator" "risk analysis" recommendation STOP "approve/reject" \
  "approved action" "execution evidence" identity "approved scope" \
  "operation id" "exit/output" "resulting state" "untrusted"; do
