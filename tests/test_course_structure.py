...
"""
MIXED_DEPENDENCY_CASES = (
    pytest.param(
        "Без платного API-ключа облачный аккаунт обязателен.",
        "требует облачную зависимость",
        id="without-paid-key-cloud-required",
    ),
    pytest.param(
        "Без облачного сервиса платный API-ключ обязателен.",
        "требует платный API-ключ",
        id="without-cloud-paid-key-required",
    ),
    pytest.param(
        "API-ключ не требуется, облачный аккаунт обязателен.",
        "требует облачную зависимость",
        id="plain-comma-without-but",
    ),
    pytest.param(
        "Облачный аккаунт обязателен, API-ключ не требуется.",
        "требует облачную зависимость",
        id="required-before-absence",
    ),
    pytest.param(
        "Без платного API-ключа\nоблачный аккаунт обязателен.",
        "требует облачную зависимость",
        id="soft-wrapped-mixed-claims",
    ),
)
...
def module_readme(dependency_claim: str) -> str:
    return f"""## Обязательный локальный маршрут
...
@pytest.mark.parametrize(("dependency_claim", "expected_error"), MIXED_DEPENDENCY_CASES)
def test_module_keeps_dependency_evidence_independent(
    tmp_path, dependency_claim, expected_error
):
    module = tmp_path / "module-01-foundations"
    write_complete_module(module, module_readme(dependency_claim))

    assert any(expected_error in error for error in validate_module(module))


def test_module_accepts_shared_absence_for_both_dependencies(tmp_path):
    module = tmp_path / "module-01-foundations"
    write_complete_module(
        module,
        module_readme("Платный API-ключ и облачный сервис не нужны."),
    )
...
@pytest.mark.parametrize(("dependency_claim", "expected_error"), MIXED_DEPENDENCY_CASES)
def test_curriculum_keeps_dependency_evidence_independent(
    tmp_path, dependency_claim, expected_error
):
    curriculum = tmp_path / "curriculum"
    write_complete_curriculum(
        curriculum,
        curriculum_readme(
            f"Прочитайте локальный Markdown-файл и запустите pytest.\n{dependency_claim}"
        ),
    )

    assert any(expected_error in error for error in validate_course(curriculum))


def test_curriculum_accepts_shared_absence_for_both_dependencies(tmp_path):
    curriculum = tmp_path / "curriculum"
    write_complete_curriculum(
        curriculum,
        curriculum_readme(
            """Прочитайте локальный Markdown-файл и запустите pytest.
Платный API-ключ и облачный сервис не нужны."""
        ),
    )
